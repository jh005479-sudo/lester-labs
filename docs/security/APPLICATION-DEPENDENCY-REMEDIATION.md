...
Six pre-remediation application files previously imported actions from
undeclared transitive `@wagmi/core`. They now use the official `wagmi/actions`
export from the direct dependency. Declaring `@wagmi/core` directly was
rejected because it was not needed and strict peer resolution attempted to add
an unused Accounts/Privy surface.

The pre-remediation source still depended on RainbowKit UI and configuration,
so the standalone dependency review replaces that removed package with a small
internal injected-wallet compatibility control. It disables multi-injected
provider discovery and remote wallet catalogues, requests LitVM chain 4441,
and disconnects if the connector returns a different chain. The later
post-compromise remediation replaces this temporary compatibility surface with
the final source-pinned wallet and transaction guard; neither review unit is a
public release on its own.

The upgraded React lint policy found nine pre-remediation errors. They were fixed
...
- invented render-time gas utilization, block, and gas-used values were removed
  instead of replacing `Math.random()` with deterministic fake data;
- the token tracker samples wall-clock time from a scheduled callback rather
  than during render; and
...
- TypeScript: pass;
- ESLint: pass with 0 errors; 108 pre-remediation warnings remain visible;
- pre-remediation application unit tests: 107/107 pass;
- Next.js 16.3.0 production build: pass, including TypeScript and generation of
  all 29 pre-remediation pages.
