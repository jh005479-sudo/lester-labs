...
| Contract lock SHA-256 | `f52df62ae65d1ec58c6732da25caf5858f5ad141ee0e1353f21dcbf770a2461d` |
| Lock shape | lockfile v3; 165 package nodes plus the root package; 2 runtime and 13 development direct declarations |
...
| Registry verification | `npm audit signatures` completed with exit 0: 140 package signatures verified and 52 registry attestations verified |
...
The remaining direct declarations were not newly introduced:
`@openzeppelin/contracts@5.1.0`, `@types/node@25.5.0`,
`typescript@5.9.3`, `@uniswap/v2-core@1.0.1`, and
`@uniswap/v2-periphery@1.1.0-beta.0`. Loose ranges on the latter four
previously ranged entries were replaced with the already selected exact
versions; `@openzeppelin/contracts` was already exact. No version change was
introduced for those five packages.

Unused direct `dotenv@17.3.1` was removed. Hardhat 3 reads the explicitly
injected process environment and the migrated configuration no longer imports
or invokes dotenv, so retaining it would add a package with environment and
filesystem access without providing a required capability.
...
disposable immutable install selected 140 packages, `npm audit` returned zero,
and `npm audit signatures` verified all 140 installed-package signatures and
52 graph attestations. TypeScript, compilation with the three pinned Solidity
compiler lines, and all 19 pre-remediation contract tests passed. No dependency security rule
was relaxed.
