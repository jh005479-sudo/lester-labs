...
describe('Vercel release orchestration', () => {
  ...
  it('cryptographically binds exact source, canary, stage, parity, approval, and promotion subjects', () => {
    ...
    assert.match(release, /safe_rollback_parity_run_id:/)
    assert.match(release, /safe-rollback-parity-run\.json/)
    assert.match(release, /emergency-served-parity\.yml", \["success"\]/)
    assert.match(release, /Failed safe rollback run lacks successful signed reconciliation/)
    assert.match(release, /malformed or truncated/)
    assert.match(release, /outputs\.safe_rollback_parity_run_id/)
